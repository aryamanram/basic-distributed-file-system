"""
ReplicatedFS Node - Core distributed file system node implementation

Integrates the membership protocol with consistent hashing, block-based
storage, replication, and consistency management to form a complete
...
# Add sibling MembershipProtocol directory to path
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'MembershipProtocol'))
...
class ReplicatedFSNode:
    """
    Main ReplicatedFS node that integrates all components.
    """
    def __init__(self, node_id: int):
        self.node_num = node_id
        self.hostname = socket.gethostname()

        # Load config to get this node's ReplicatedFS port
        ...
        self.port = node_config['rfs_port']

        # Initialize logger
        self.logger = Logger(node_id)

        # Initialize membership service to get consistent member_id
            ...
            try:
                ...
                for hostname, info in members.items():
                    if info['status'] == 'ACTIVE':
                        ...
                        member_node_id = f"{hostname}:{self.port}:{info['id'].split('_')[0]}"
    ...
    def _handle_list_mem_ids(self, message: dict, addr: tuple) -> dict:
        ...
        for hostname, info in sorted(members.items()):
            if info['status'] in ['ACTIVE', 'JOINING']:
                node_id = f"{hostname}:{self.port}:{info['id'].split('_')[0]}"
                ring_pos = self.ring.get_node_position(node_id)
                member_info.append({
                    'hostname': hostname,
                    'node_id': node_id,
