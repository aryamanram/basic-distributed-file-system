"""
ReplicatedFS Main - Command Line Interface with Control Server
"""
import argparse
import json
import os
...
from .node import ReplicatedFSNode
...
class ReplicatedFSClient:
    """
    Client interface for ReplicatedFS operations.
    """
    def __init__(self, node: ReplicatedFSNode):
        self.node = node
        self.client_id = f"{node.hostname}_{os.getpid()}_{time.time()}"

    def create(self, local_filename: str, rfs_filename: str):
        """
        Create a file on ReplicatedFS.
        """
        print(f"Creating {rfs_filename} from {local_filename}...")
        ...
        # Get replica nodes
        replicas = self.node.get_replicas_for_file(rfs_filename)
    ...
    def get(self, rfs_filename: str, local_filename: str):
        """
        Get a file from ReplicatedFS.
        """
        print(f"Getting {rfs_filename} to {local_filename}...")
    ...
    def append(self, local_filename: str, rfs_filename: str):
        """
        Append to a file on ReplicatedFS.
        """
        print(f"Appending {local_filename} to {rfs_filename}...")
        ...
        self.node.consistency.mark_write_pending(rfs_filename, self.client_id)
        ...
        try:
            ...
            # Create block
            block_id = f"{get_file_id(rfs_filename)}_{self.client_id}_{seq_num}"
            block = FileBlock(
                block_id=block_id,
                client_id=self.client_id,
            ...
            self.node.consistency.mark_write_complete(rfs_filename, self.client_id)

    def merge(self, rfs_filename: str):
        """
        Merge all replicas of a file.
        """
        print(f"Merging {rfs_filename}...")
    ...
    def ls(self, rfs_filename: str):
        """
        List machines storing a file.
        """
        print(f"Listing replicas for {rfs_filename}...")
    ...
    def getfromreplica(self, node_address: str, rfs_filename: str, local_filename: str):
        """
        Get file from a specific replica.
        """
        print(f"Getting {rfs_filename} from {node_address} to {local_filename}...")
    ...
    def multiappend(self, rfs_filename: str, node_addresses: list, local_filenames: list):
        """
        Launch multiple appends simultaneously.
        """
        print(f"Launching multi-append to {rfs_filename}...")
        print(f"Nodes: {node_addresses}")
        print(f"Files: {local_filenames}")

        def append_from_node(node_addr, local_file):
            temp_client = ReplicatedFSClient(self.node)
            temp_client.client_id = f"{node_addr}_{os.getpid()}_{time.time()}"
            temp_client.append(local_file, rfs_filename)

        threads = []
        for node_addr, local_file in zip(node_addresses, local_filenames):
            thread = threading.Thread(target=append_from_node, args=(node_addr, local_file))
            thread.start()
...
def execute_command(client: ReplicatedFSClient, cmd: str) -> str:
    """
    Execute a ReplicatedFS command and return the result as a string.
        ...
        try:
            ...
            if command == 'create' and len(parts) == 3:
                ...
                if len(nodes) == len(files):
                    client.multiappend(parts[1], nodes, files)
                else:
                    print("Error: Number of nodes and files must match")

            elif command == 'status':
                print(f"OK - Node-{client.node.node_num}")
...
def control_server_loop(client: ReplicatedFSClient, control_port: int, logger):
    """
    ...
    try:
        ...
        logger.log(f"CONTROL: Server listening on port {control_port}")
...
def main():
    parser = argparse.ArgumentParser(description='ReplicatedFS - Replicated Distributed File System')
    parser.add_argument('--node-id', type=int, required=True, help='Node ID (matches config.json)')
    parser.add_argument('--no-interactive', action='store_true',
                       help='Disable interactive mode (control server only)')

    args = parser.parse_args()

    # Load config to get control port
    ...
    if control_port is None:
        print(f"Error: Node ID {args.node_id} not found in config.json")
        sys.exit(1)

    # Create and start node
    node = ReplicatedFSNode(args.node_id)
    node.start()
    ...
    # Create client
    client = ReplicatedFSClient(node)

    # Start control server in background
    control_thread = threading.Thread(
        target=control_server_loop,
        args=(client, control_port, node.logger),
        daemon=True
    )
    ...
    if args.no_interactive:
        ...
        # Interactive mode
        print("\n" + "="*60)
        print("ReplicatedFS Command Interface")
        print("="*60)
        print("Commands:")
