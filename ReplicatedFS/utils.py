"""
Utility functions for ReplicatedFS
"""
import hashlib
import json
import os
...
class Logger:
    """
    Logger for ReplicatedFS operations.
    """
    def __init__(self, node_id: int, log_file: Optional[str] = None):
        self.node_id = node_id
        os.makedirs("logs", exist_ok=True)
        self.log_file = log_file or f"logs/machine.{node_id}.log"
    ...
    def log(self, message: str):
        ...
        log_line = f"[{timestamp}] [Node-{self.node_id}] {message}"
        print(log_line)
        try:
...
def get_file_id(filename: str) -> str:
    """
    Generate a unique file ID for a ReplicatedFS filename.
    """
